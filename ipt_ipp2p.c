...
#define get_u8(X,O)  (*(uint8_t *)(X + O))
#define get_u16(X,O)  (*(uint16_t *)(X + O))
#define get_u32(X,O)  (*(uint32_t *)(X + O))
...
search_ares (unsigned char *haystack, int packet_len, int head_len)
{
	...
	{
		const uint16_t size=packet_len - head_len;
		switch (t[2])
		{
			case 0x5a:
				/* ares connect */
...
static struct {
    int command;
    uint8_t short_hand;			/*for fucntions included in short hands*/
    int packet_len;
...
int
scan(unsigned char *buf, int len)
{
unsigned char *haystack = NULL;
struct ip *ip = (struct ip *) buf;
int head_len, i = 0, p2p_result = 0, mc = 0;
int hlen = ntohs(ip->ip_len)-(ip->ip_hl)*4; /* hlen = packet data length */

#if 0 /* XXX */
if (ip->ip_off!=0) /* must not be a fragment */
  return 0;
#endif

haystack=(char *)ip+(ip->ip_hl)*4;

switch(ip->ip_p) {
  case IPPROTO_TCP:
  {
    struct tcphdr *tcph = (struct tcphdr *) ((unsigned char *)ip + (ip->ip_hl) * 4);

    if ((tcph->th_flags & TH_FIN) || /* if FIN bit is set, */ 
	(tcph->th_flags & TH_SYN) || /* or SYN bit is set, */
	(tcph->th_flags & TH_RST))   /* or RST bit is set, */
       return 0;                     /* bail out */
    head_len = tcph->th_off * 4;     /* get the TCP header size */
		...
		if (/*(((info->cmd & matchlist[i].command) == matchlist[i].command) ||
		    ...
		    (hlen > matchlist[i].packet_len)) {
			    ...
			    {
#if 0 /* Commented out */
				if (debug) printf("IPP2P.debug:TCP-match: %i from: %u.%u.%u.%u:%i to: %u.%u.%u.%u:%i Length: %i\n", 
	    ...
	    if (mc > 1) pktst.ambigous++;
	    return 0;
  }
  case IPPROTO_UDP:
  {
#if 0 /* Commented out */
    struct udphdr *udph = (struct udphdr *) ((unsigned char *) ip + (ip->ip_hl) * 4);
#endif
		...
		if (/*(((info->cmd & udp_list[i].command) == udp_list[i].command) ||
		    ...
		    (hlen > udp_list[i].packet_len)) {
			    ...
			    if (p2p_result){
#if 0 /* Commented out */
				if (debug) printf("IPP2P.debug:UDP-match: %i from: %u.%u.%u.%u:%i to: %u.%u.%u.%u:%i Length: %i\n", 
				    p2p_result, NIPQUAD(ip->saddr),ntohs(udph->source), NIPQUAD(ip->daddr),ntohs(udph->dest),hlen);
#endif
				pktst.clstats[i|0x10]++;
				mc++;
			    }
		}
	    i++;
